...
QVariant* FeatureCoverage::checkInput(QVariant* inputVar, quint32 columnIndex) const{
    QVariant qVar = this->ptr()->as<Ilwis::FeatureCoverage>()->attributeDefinitionsRef().checkInput(*inputVar, columnIndex);
    return new QVariant(qVar); //TODO delete
}
...
FeatureCoverage *FeatureCoverage::toFeatureCoverage(IObject *obj){
